...
describe("contribution client", () => {
  it("calls contribution endpoints with session token header", async () => {
    ...
    expect(calls[0]?.path).toBe("/api/v1/posts");
    expect(calls[1]?.path).toBe("/api/v1/posts/post_1/applications");
    expect(calls[2]?.path).toBe("/api/v1/posts/post_1/applications/app_1/accept");
    expect(
      calls.every(
