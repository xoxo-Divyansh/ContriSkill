...
class FakeTransport implements RealtimeTransportLifecycle {
  ...
  emitUpgradeWithReject(url: string, reject: (statusCode: number, message: string) => void): void {
    this.upgradeHandler?.({
      request: {
        url
      } as never,
...
describe("realtime runtime foundation", () => {
  it("authenticates connection and supports subscription primitives", async () => {
    ...
    runtime.start();
    transport.emitUpgrade("/api/v1/realtime?accessToken=tok_123");
  ...
  it("rejects unauthenticated upgrades", async () => {
    ...
    transport.emitUpgradeWithReject("/api/v1/realtime", rejectSpy);
    await Promise.resolve();
